...
public class BaseConverter {
	...
	public static void main(String[] args) {
		...
		while (again == 0) {
			...
			number = JOptionPane.showInputDialog("Enter the number to convert: ").toUpperCase(); //Make sure all letters are uppercase
			...
			if (conversion == 6) { //Variable base to another variable base was selected so we need a bit more info
				...
				firstBase = Integer.parseInt(bases.substring(0, bases.indexOf(" ")).replaceAll("\\s+", ""));
				secondBase = Integer.parseInt(bases.substring(bases.indexOf(" ") + 1).replaceAll("\\s+", ""));
			}
			
			while(checker.errorCheck(conversion, number, firstBase)) {
